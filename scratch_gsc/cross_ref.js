...
console.log("Analyzing Duplicate Pages Performance...\n");

for (const slug of duplicateSlugs) {
  const url = `https://tattoosmap.com/gallery/${slug}`;
  const row = lines.find(line => line.startsWith(url + ','));
  if (row) {
    const parts = row.split(',');
    console.log(`URL: ${url}`);
    console.log(`Clicks: ${parts[1]}, Impressions: ${parts[2]}, Position: ${parts[4]}`);
    console.log('---');
  }
}
